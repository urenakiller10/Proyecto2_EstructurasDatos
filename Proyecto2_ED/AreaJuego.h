...
namespace Proyecto2_ED {
    ...
    private:
        ...
        ListaSimpleAvl* listaAvl = new ListaSimpleAvl();
...
#pragma region Windows Form Designer generated code
        ...
        void actualizarInfo()
        {
            ...
            for (int fila = 0; fila < TablaJuego->Rows->Count; fila++)
            {
                ...
                NodoListaAvl* tmpA = listaAvl->primerNodo;
                while (tmpA != nullptr) {
                    ArbolAvl* arbolAv = tmpA->dato;
                    if (arbolAv->x == x && arbolAv->y == y) {
                        // Capturar los datos y actualizar las celdas correspondientes
                        std::string cantidadFrutos = std::to_string(arbolAv->calcularTotalNodos());
                        System::String^ canFrutos = gcnew System::String(cantidadFrutos.c_str());
                        dataGridViewRow->Cells[2]->Value = canFrutos;

                        std::string sumaFrutos = std::to_string(arbolAv->calcularSumaValores());
                        System::String^ sumFrutos = gcnew System::String(sumaFrutos.c_str());
                        dataGridViewRow->Cells[3]->Value = sumFrutos;
        ...
        void hiloAvl() {
            ArbolAvl* a = new ArbolAvl(labelGranjero->Top, labelGranjero->Left);
            listaAvl->agregarElemento(a);
            // Agregar el árbol binario a la lista
            if (a->listo == false) {
                System::Threading::Thread::Sleep(config->getCreceAvl() * 1000);
                a->listo = true;
            }

            while (running) {
                srand(time(NULL));
                float fruto = config->getMinValue() + static_cast<float>(rand()) * static_cast<float>(config->getPrecioFrutosAvl() - config->getMinValue()) / RAND_MAX;
                System::Threading::Thread::Sleep(config->getCosechaAvl() * 1000);
                a->insertar(fruto);
            }
        }
        ...
        void InitializeComponent(void)
        {
            ...
            this->TablaJuego->Location = System::Drawing::Point(1000, 276);
            ...
            this->TablaJuego->ScrollBars = System::Windows::Forms::ScrollBars::Vertical;
            this->TablaJuego->ShowRowErrors = false;
            this->TablaJuego->Size = System::Drawing::Size(810, 31);
            ...
            this->B_VenderTodo->Font = (gcnew System::Drawing::Font(L"Microsoft Sans Serif", 12, System::Drawing::FontStyle::Bold, System::Drawing::GraphicsUnit::Point,
                static_cast<System::Byte>(0)));
            this->B_VenderTodo->Location = System::Drawing::Point(1269, 473);
            ...
            this->ClientSize = System::Drawing::Size(1860, 844);
...
#pragma endregion
    ...
    void B_GuardarJuego_Click(Object^ sender, EventArgs^ e)
    {
        ...
        if (inputForm->ShowDialog() == System::Windows::Forms::DialogResult::OK) {
            ...
            // Abrir el archivo en modo de escritura y adjuntar al final
            std::ofstream archivo("Top10.txt", std::ios::app);

            // Verificar si el archivo se abrió correctamente
            if (archivo.is_open()) {
                // Escribir en el archivo
                archivo << std::left << std::setw(20) << nombreStr << std::right << std::setw(10) << dinero << std::endl;
...
private: System::Void B_PlantarAVL_Click(System::Object^ sender, System::EventArgs^ e) {
    ...
    if (labelGranjero->BackColor != Color::Gray && labelGranjero->BackColor != Color::LimeGreen)
    {
        ...
        //Crear el hilo
        Thread^ nuevoHiloAvl = gcnew Thread(gcnew System::Threading::ThreadStart(this, &AreaJuego::hiloAvl));
        nuevoHiloAvl->Start();

        //Agregar el hilo a la lista
        listaHilosAvl->Add(nuevoHiloAvl);
        ActualizarInformacionNuevo("Arbol Avl", granjeroX, granjeroY);
...
private: System::Void B_Salir_Click(System::Object^ sender, System::EventArgs^ e) {
    ...
    private: System::Void B_VenderTodo_Click(System::Object^ sender, System::EventArgs^ e) {
        ...
        NodoListaAvl* tmpAA = listaAvl->primerNodo;
        while (tmpAA != nullptr) {
            ArbolAvl* arbolA = tmpAA->dato;
            suma += arbolA->calcularSumaValores();
            arbolA->raiz = NULL;
            tmpAA = tmpAA->siguiente;
        }
